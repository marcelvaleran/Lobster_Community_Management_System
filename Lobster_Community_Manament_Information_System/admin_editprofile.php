...
<form class="wrapper mt-sm-5" method="POST" enctype="multipart/form-data">
    <div class="pb-4 border-bottom">
        ...
        <label class="rightset"><i class="fas fa-calendar-week" style="margin-left: 4.2em;"></i></label>
        </h4>
    </div>
    ...
    <div class="py-2">
        ...
        <div class="row py-2">
            ...
            <div class="col-md-6 pt-md-0 pt-3" id="lang"> <label for="language">Account Status</label>
                <div class="arrow"> 
                    <input type="text" name="activation" class="bg-light form-control" value="<?php echo $activation; ?>" readonly>
                </div>
            </div>
        </div>
        <div class="py-3 pb-4 border-bottom">
            <input class="btn btn-primary mr-3" type="submit" name="submit" value="Save Changes" />
            <a href="admin_listuser.php" class="btn border button">Back</a>
        </div>
        <div class="d-sm-flex align-items-center pt-3" id="deactivate">
            <div> 
                <b>Deactivate user account</b>
                <p>Details about your member account, contact and address</p>
            </div>
            <div class="ml-auto">
                <?php 
                if ($activation == "Deactivated") { ?>
                    <button name="submit2" class="btn btn-outline-success" onclick="return confirm('Activate the account?')">Activate</button>
                <?php }

                if ($activation == "Active") { ?>
                    <button name="submit2" class="btn btn-outline-danger" onclick="return confirm('User cannot login anymore until you activate it again. Proceed?')">Deactivate</button>
                <?php } ?>
            </div>
        </div>
    </div>
...
if(isset($_POST['submit2'])) {
    ...
    $pass = "";
    $database = "database";
    $conn4 = mysqli_connect($server, $user, $pass, $database) or die(mysqli_error());
    ...
    if ($row['level'] == "Admin") {
        echo "<script>alert('Administrator account cannot be deactivated!')</script>";
    } else {
    ...
    $res4 = mysqli_query($conn4, $sql4);
     ...
     if($res4==true)
     {
        echo "<script>alert('Account activation updated.')</script>";
        echo "<script>window.history.back();</script>";
     } else{ 
