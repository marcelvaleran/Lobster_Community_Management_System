        ...
        <nav class="nav">
            <div> <a href="#" class="nav_logo"> <i class='bx bx-layer nav_logo-icon'></i> <span class="nav_logo-name">Menu</span> </a>
                <div class="nav_list"> 
                    ...
                    <a href="admin_sales_record.php" class="nav_link active"> 
    ...
    <div style="border: 1px solid #fff; border-radius: 8px;">
         <div class="card" style="border-radius: 8px; background: rgba(45, 98, 245, 0.2);">
            ...
            <form method="POST" style="margin-top: 0;">
                ...
                <select id="filter-sizee" name="category" class="filter form-select mr-2 ml-1" style="float: right; width: 18%; background: transparent; color: #fff;">
                ...
                <a class="btn btn-primary mr-1" title="New Transaction Data" href="admin_sale.php"><i class="fa fa-plus-circle fa-lg"></i></a>
            </form>
            </div>
...
$query=mysqli_query($conn, "SELECT * FROM sale WHERE `size`='$category' AND `user_id2`='$user_id'") or die(mysqli_error($conn));
                            $qty = 0;
...
<a href="#" type="button" class="badge badge-success" style="padding: 7px" data-toggle="modal" data-target="#views<?php echo $row['item_id']; ?>" title="View"><i class="fas fa-info-circle" title="View"></i> View</a>
                            ...
                            $query=mysqli_query($conn, "SELECT * FROM sale WHERE `user_id2`='$user_id' ORDER BY item_id ASC") or die(mysqli_error());
                            $qty = 0;
                            while($row=mysqli_fetch_array($query)){
                                $item_id = $row['item_id'];
                                $seller = $row['seller'];
...
<a href="#" type="button" class="badge badge-success" style="padding: 7px" data-toggle="modal" data-target="#views<?php echo $row['item_id']; ?>" title="View"><i class="fas fa-info-circle" title="View"></i> View</a>
                            ...
                            $query=mysqli_query($conn, "SELECT * FROM sale WHERE `user_id2`='$user_id' ORDER BY item_id ASC") or die(mysqli_error($conn));
                            $qty = 0;
                            while($row=mysqli_fetch_array($query)){
                                $item_id = $row['item_id'];
                                $seller = $row['seller'];
                    ...
                    <form class="row register-form" >
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <select name="pay_method" class="form-control" readonly disabled>
