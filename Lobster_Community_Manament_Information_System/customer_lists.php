    ...
    <div style="border: 1px solid #fff; border-radius: 8px;">
         <div class="card" style="border-radius: 8px; background: rgba(45, 98, 245, 0.2);">
            ...
            <div style="overflow: hidden; margin-bottom: -1.7em; width: 70%; float: right; position: relative; z-index: 1;">
                ...
                <a class="btn btn-primary mr-1" title="Request Customer Data" href="#" data-toggle="modal" data-target="#addData"><i class="fa fa-user-plus fa-lg"></i></a>
                <a class="btn btn-primary mr-1" title="Request List" href="customer_req.php"><i class="fa fa-tasks fa-lg"></i></a>
        ...
        <?php
            ...
            $conn = mysqli_connect($server, $user, $pass, $database) or die(mysqli_error());
                ...
                $sql = "SELECT * FROM customer WHERE request = 'Approved'";
                    ...
                    if($count_rows>0)
                    {
                        ...
                        while($row=mysqli_fetch_assoc($res))
                        {
                            ...
                            <tr align='center' style="background: transparent;">
                                ...
                                <td>
            <a href="#" class="badge badge-primary" style="padding: 7px" data-toggle="modal" data-target="#views<?php echo $row['cust_id']; ?>"><i class="fas fa-info-circle" title="View"></i>  View</a>
                                </td>
                            </tr>
                            <?php
                            $sno++;
            ...
            <div class="tab-content" id="myTabContent">
                    <form class="row register-form" >
                        ...
                        <div class="form-group">
                         ...
                         <div class="form-group">
                            <label for="lastname"><i class="fas fa-id-card-alt mr-2"></i> ID Card</label>
                            <input type="text" class="form-control bold" value="<?php echo $id_card; ?>"readonly/>
                        </div>
                        <div class="form-group">
...
<div class="modal fade" id="addData" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color: rgba(9, 95, 219, 1); ">
        <h3 class="modal-title"><i class="fas fa-users" title="View"></i> Request Customer Data</h3>
            <button type="button" class="close btn mr-1" data-dismiss="modal" aria-label="Close">
              <i class="fas fa-times fa-lg"></i>
            </button>
      </div>
            ...
            <div class="tab-content" id="myTabContent">
                    <form action="" method="POST" class="row register-form" >
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="id_card" list="suggest" id="gets" onkeyup="chk_db()" class="form-control bold" required />
                        </div>
                        <div class="form-group">
                            <label for="lastname"><i class="fas fa-users mr-2"></i> Customer<span class="text-danger"> *</span></label>
                            <input type="text" name="buyer_name" id="cust_name" class="form-control bold" required />
                        </div>
                        <div class="form-group">
                    ...
                    <div class="modal-footer">
                        <button name="submit" class="btn btn-success"><i class="fas fa-plus-circle"></i> Request</button>
                        <button type="button" class="btn btn-danger" data-dismiss="modal"><i class="fas fa-times-circle"></i> Close</button>
                    </div>
                    </form>
                </div>
...
<datalist id="suggest">
    <?php
        $sql1=mysqli_query($conn,"SELECT * FROM customer ORDER BY cust_id ASC");
        while ($data = mysqli_fetch_array($sql1)){
            echo "<option value=".$data['id_card'].">".$data['cust_name']."</option>";
    }?>
</datalist>
  <script>
  function chk_db(){
...
if (isset($_POST['submit'])) {
    ...
    $duplicate = mysqli_query($conn,"SELECT * FROM customer WHERE id_card = $id_card");

    if (mysqli_num_rows($duplicate)>0) {
        echo "<script>alert('This customer data already existed or requested!')</script>";
        echo "<script>
    ...
    $sql2 = "INSERT INTO customer (user_id6, cust_name, id_card, phoneNo, email, address, post_code, subdistrict, create_date, request)
    VALUES ('$user_id', '$buyer_name', $id_card, '$phoneNo', '$email', '$address', '$post_code', '$subdistrict', '$create_date', '$request')";
