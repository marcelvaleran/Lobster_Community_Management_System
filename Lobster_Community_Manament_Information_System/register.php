...
if (isset($_POST['submit'])) {
    ...
    if ($password == $cpassword) {
        ...
        if (!$result->num_rows > 0) {
            $sql = "INSERT INTO users (username, email, password, full_name, address, subdistrict, phone, level, post_code, create_datetime, activation)
                    VALUES ('$username', '$email', '$password', '$full_name', '$address', '$subdistrict', '$phone', '$level', '$post_code', '$currentDateTime', '$activation')";
            $result = mysqli_query($conn, $sql);
            if ($result) {
                ...
                <form class="form-card" action="" method="POST">
                    ...
                    <div class="row justify-content-between text-left">
                        ...
                        <div class="form-group col-sm-6 flex-column d-flex"> 
                            ...
                            <input type="password" id="mob" placeholder="Password" name="password" value="<?php echo $_POST['password']; ?>" onblur="validate(3)" required> 
                        </div>
                    </div>
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-sm-6 flex-column d-flex"> 
                        ...
                        <div class="form-group col-sm-6 flex-column d-flex"> 
                            ...
                            <input type="phone" id="job" placeholder="Phone/WA" name="phone" value="<?php echo $_POST['phone']; ?>" onblur="validate(5)" required> 
                        </div>
                    </div>
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-sm-6 flex-column d-flex"> 
                            <label class="form-control-label px-3">Sub-district<span class="text-danger"> *</span></label> 
                            <input type="text" id="job" placeholder="Sub-district" name="subdistrict" list="suggest" value="<?php echo $_POST['subdistrict']; ?>" onblur="validate(5)" required> 
                        </div>
                        <div class="form-group col-sm-6 flex-column d-flex"> 
                            <label class="form-control-label px-3">Postal Code<span class="text-danger"> *</span></label> 
                            <input type="text" id="job" placeholder="Zip" name="post_code" value="<?php echo $_POST['post_code']; ?>" onblur="validate(5)" required> 
                        </div>
                    </div>
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-12 flex-column d-flex"> 
                            <label class="form-control-label px-3">Address<span class="text-danger"> *</span></label> 
                            <input type="address" id="ans" placeholder="Address" name="address" value="<?php echo $_POST['address']; ?>" onblur="validate(6)" required> </div>
                        </div>
